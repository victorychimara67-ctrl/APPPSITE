...
async function createStripeCheckoutSession(req, order) {
  if (!STRIPE_SECRET_KEY) throw new Error("Stripe secret key is not configured on the server.");
  const amount = stripeAmount(order.total);
  if (amount <= 0) throw new Error("Stripe requires an order total greater than zero.");
  ...
  form.set("cancel_url", `${origin}/?payment=cancelled&order=${encodeURIComponent(order.id)}#cart`);
  form.set("customer_email", order.recipient.email);
  form.set("line_items[0][quantity]", "1");
  form.set("line_items[0][price_data][currency]", order.currency.toLowerCase());
  form.set("line_items[0][price_data][unit_amount]", String(amount));
  form.set("line_items[0][price_data][product_data][name]", "ECI Custom Order");
  form.set("line_items[0][price_data][product_data][description]", `${order.items.length} Emmanuel CI Universe item${order.items.length === 1 ? "" : "s"}`);
